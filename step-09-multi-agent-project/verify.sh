#!/bin/bash
# Step 09: 멀티 에이전트 협업 실습 환경 검증 스크립트
# CAO 서버 실행 상태, 에이전트 프로필 설치 여부, 샘플 프로젝트 디렉토리 존재를 확인한다
...
PROFILES=("code_supervisor" "developer" "reviewer" "tester")
...
for profile in "${PROFILES[@]}"; do
  ...
  → cao install profiles/${profile}.md"
    fi
done

# 3. sample-project 디렉토리 존재 확인
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SAMPLE_DIR="$SCRIPT_DIR/sample-project"

if [ -d "$SAMPLE_DIR" ]; then
    print_pass "sample-project 디렉토리" "존재함"
else
    print_fail "sample-project 디렉토리" "sample-project 디렉토리를 찾을 수 없습니다 ($SAMPLE_DIR)"
fi
